...
export function DatePicker({ value, onChange, placeholder = "DD-MM-YYYY" }: DatePickerProps) {
  ...
  const isValuePastDate = (val: string) => {
  ...
  const inputHasError = touched && value.length === 10 && isValuePastDate(value)
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      ...
      cells.push(
        <button
          ...
          disabled={past}
          className={cn(
            "h-8 w-8 text-sm font-medium rounded-full font-sans transition-all flex items-center justify-center",
            past
              ? "text-[#A9A9A9] opacity-40 cursor-default"
              : selected
                ? "bg-[#303030] text-white font-semibold cursor-pointer active:scale-90"
                : currentToday
                  ? "border border-[#FF5623] text-[#FF5623] font-semibold hover:bg-[#FF5623]/5 cursor-pointer active:scale-90"
                  : "text-[#303030] hover:bg-gray-100 cursor-pointer active:scale-90"
          )}
        >
          {d}
        </button>
  ...
  return (
    <div className="relative w-full" ref={containerRef}>
      <input
        ...
        onBlur={handleBlur}
        className={cn(
          "w-full h-12 pl-4 pr-12 rounded-full border focus:outline-none focus:ring-1 focus:ring-black text-base font-sans text-[#303030] placeholder-[#A9A9A9] transition-all",
          inputHasError
